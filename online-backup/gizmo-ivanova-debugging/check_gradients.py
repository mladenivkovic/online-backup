                ...
                if py < sw:
                    ...
                    for n in range(nneigh_s[p]):
                        if swn[n] not in pyn:
                            ...
                            if r / H[p] > 1.0 and r / H[p] < 1 + 1e-3:
                                print(
                                    "removing neighbour {0:7d} from particle {1:7d}. r/H = {2:14.7f}".format(
                                        ids[nind], ids[p], r / H[p]
                                    )
                                )
                                to_remove.append(n)
                    ...
                    if len(to_remove) > 0:
                        for i, N in enumerate(to_remove):
                            ...
                            nids_s[p][n : nneigh_s[p] - 1] = nids_s[p][
                                n + 1 : nneigh_s[p]
                            ]
                            nneigh_s[p] -= 1
                            print(nids_s[p][: nneigh_s[p]])
            ...
            if py != sw:
                ...
                for n in range(nl):
                    if larger[n] not in smaller:
                        ...
                        if abs(1 - r / H[p]) < 1e-3:
                            print(
                                "removing neighbour {0:7d} from particle {1:7d}. r/H = {2:14.7f}, dataset={3:10}".format(
                                    ids[nind], ids[p], r / H[p], larger_name
                                )
                            )
                            to_remove.append(n)
                        else:
                            print(
                                "particle's {0:7d} neighbour {1:7d} has something weird happening: r/H = {2:14.7f}".format(
                                    ids[p], ids[nind], r / H[p]
                                )
                            )
        ...
        for p in range(npart):
            ...
            for n in range(nneigh_s[p]):
                ...
                if py != sw:
                    ...
                    print(
                        "In neighbour IDs: found difference for particle ID {0:6d}, index {1:6d}".format(
                            ids[p], p
                        )
                    )
                    print("sw:", swarr)
        ...
        for p in range(npart):
            ...
            while True:
                ...
                if diff > tolerance:
                    ...
                    print(
                        (
                            " Particle ID {0:8d}, position {1:14.7e} {2:14.7e}, h = {3:14.7e}, H = {4:14.7e}, dist = {5:14.7e}, dist/H = {6:14.7e}"
                        ).format(ids[p], pos[p, 0], pos[p, 1], h[p], H[p], r, r / H[p])
                    )
                    print(
                        (
                            "Neighbour ID {0:8d}, position {1:14.7e} {2:14.7e}, h = {3:14.7e}, H = {4:14.7e}"
                        ).format(
                            ids[nind], pos[nind, 0], pos[nind, 1], h[nind], H[nind]
                        )
                    )
                    print(
                        "Wj(xi) py = {0:14.7e}, Wj(xi) swift = {1:14.7e}, diff = {2:12.6f}".format(
                            py, sw, diff
                        )
                    )
        ...
        for p in range(npart):
            ...
            for P, S in [(pyv, swv), (pyn, swn)]:
                ...
                if diff > tolerance:
                    ...
                    print(
                        "volume:    {0:14.7e}  {1:14.7e}  {2:14.7f}".format(
                            pyv, swv, abs(1 - pyv / swv)
                        )
                    )
                    print(
                        "norm:      {0:14.7e}  {1:14.7e}  {2:14.7f}".format(
            ...
            if do_break and found_difference:
                break
        print("Volume sums:")
        print("Python: {0:14.7f}".format(np.sum(1.0 / omega_p)))
        ...
        for p in range(npart):
            ...
            while True:
                ...
                if diff > tolerance:
                    ...
                    print(
                        "dwdr py = {0:14.7e}, dwdr swift = {1:14.7e}, diff = {2:12.6f}".format(
                            py, sw, diff
                        )
                    )
                ...
                if diff > tolerance:
                    ...
                    print(
                        (
                            ...
                            + "H = {4:14.7e}, dist = {5:14.7e}, dist/H = {6:14.7e}"
                        ).format(ids[p], pos[p, 0], pos[p, 1], h[p], H[p], r, r / H[p])
                    )
                    print(
                        (
                    ...
                    print(
                        "r py = {0:14.7e}, r swift = {1:14.7e}, 1 - swift/py = {2:12.6f}".format(
                            py, sw, 1 - sw / py
                        )
                    )
                    print(r)
            ...
            while True:
                ...
                for P, S in [(pyx, swx), (pyy, swy)]:
                    ...
                    if diff > tolerance:
                        ...
                        print("In dx:")
                        print(
                            "Found difference:  ID: {0:14d} neighbour ID: {1:14d}".format(
                                ids[p], nids_p[p, n]
                            )
                        )
                        #  print("Found difference:  ID: {0:14d} neighbour ID: {1:14d} difference: {2:12.6f} r/H: {3:14.7e}".format(ids[p], nids_p[p,n], diff, r/H[p]))
                        print(
                            "       positions:   x:  {0:14.7e}  {1:14.7e}".format(
                                pos[p][0], pos[nind][0]
                            )
                        )
                        print(
                            "       positions:   y:  {0:14.7e}  {1:14.7e}".format(
                                pos[p][1], pos[nind][1]
                            )
                        )
                        print(
                            "       dx:         python:                      swift:                    python/swift:"
                        )
                        print(
                            "                   x: {0:14.7e}           {1:14.7e}            {2:14.7e}".format(
                                pyx, swx, pyx / swx
                            )
                        )
                        print(
                            "                   y: {0:14.7e}           {1:14.7e}            {2:14.7e}".format(
                                pyy, swy, pyy / swy
                            )
                        )
                        print()
            ...
            while True:
                ...
                for P, S in [(pyx, swx), (pyy, swy)]:
                    ...
                    if diff > tolerance:
                        ...
                        print(
                            "Found difference:     ID: {0:14d}  neighbour {1:14d} difference: {2:12.6f} r/H: {3:14.7e}\n".format(
                                ids[p], nids_p[p, nip], diff, r / H[p]
                            )
                        )
                        print(
                        ...
                        print(
                            " gradient contribution x: {0:14.7e}           {1:14.7e}          {2:14.7f}".format(
                                pyx, swx, pyx / swx
                            )
                        )
                        print(
                            " gradient contribution y: {0:14.7e}           {1:14.7e}          {2:14.7f}".format(
                                pyy, swy, pyy / swy
                            )
                        )
                        ...
                        S = dx_s[p, 2 * nis]
                        print(
                            "                      dx: {0:14.7e}           {1:14.7e}          {2:14.7f}".format(
                                P, S, P / S
                            )
                        )
                        P = dx_p[p, nip, 1]
                        S = dx_s[p, 2 * nis + 1]
                        print(
                            "                      dy: {0:14.7e}           {1:14.7e}          {2:14.7f}".format(
                                P, S, P / S
                            )
                        )
        ...
        for p in range(npart):
            ...
            for P, S in [(pyx, swx), (pyy, swy)]:
                ...
                if diff > tolerance:
                    ...
                    print(
                        "Found difference: ID: {0:14d} difference: {1:12.6f}".format(
                            ids[p], diff
                        )
                    )
                    print(
                    ...
                    print(
                        "              sum  x: {0:14.7e}         {1:14.7e}         {2:14.7f}".format(
                            pyx, swx, pyx / swx
                        )
                    )
                    print(
                        "              sum  y: {0:14.7e}         {1:14.7e}         {2:14.7f}".format(
                            pyy, swy, pyy / swy
                        )
                    )
                    ...
                    print(
                        "              sum  x: {0:14.7e}         {1:14.7e}".format(
                            repy[0], reswx
                        )
                    )
                    print(
                        "              sum  y: {0:14.7e}         {1:14.7e}".format(
                            repy[1], reswy
                        )
                    )
            ...
            while True:
                ...
                for P, S in [(pyx, swx), (pyy, swy)]:
                    ...
                    if diff > tolerance:
                        ...
                        print(
                            "       positions:   x:  {0:14.4f}  {1:14.4f}".format(
                                pos[p][0], pos[nind][0]
                            )
                        )
                        print(
                            "       positions:   y:  {0:14.4f}  {1:14.4f}".format(
                                pos[p][1], pos[nind][1]
                            )
                        )
                        print()
                        print(
                            "       del psi/del x:  python:         swift:              1-python/swift:"
                        )
                        print(
                            "                   x: {0:14.7e}  {1:14.7e}  {2:14.7f}".format(
                                pyx, swx, 1 - pyx / swx
                            )
                        )
                        print(
                            "                   y: {0:14.7e}  {1:14.7e}  {2:14.7f}".format(
                                pyy, swy, 1 - pyy / swy
                            )
                        )
                        print()
                        ...
                        print()
                        print(
                            "Values are ignored below {0:14.7e}".format(
                                maxgrad * NULL_RELATIVE
                            )
                        )
