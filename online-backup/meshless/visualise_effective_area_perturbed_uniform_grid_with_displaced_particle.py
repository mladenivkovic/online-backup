...
def main():
    ...
    # scatter neighbours
    ax1.scatter(xp[mask], yp[mask], s=ps, lw=lw, facecolor=fc, edgecolor=ec)
    ax2.scatter(xp[mask], yp[mask], s=ps, lw=lw, facecolor=fc, edgecolor=ec)
    ax3.scatter(xp[mask], yp[mask], s=ps, lw=lw, facecolor=fc, edgecolor=ec)

    # scatter central
    ax1.scatter(x[cind], y[cind], s=ps, lw=lw, facecolor=ec, edgecolor=ec)
    ax2.scatter(x[cind], y[cind], s=ps, lw=lw, facecolor=ec, edgecolor=ec)
    ax3.scatter(x[cind], y[cind], s=ps, lw=lw, facecolor=ec, edgecolor=ec)
    ...
    fig.suptitle(
        r"Effective Area $\mathbf{A}_{ij}$ of a particle w.r.t. the central particle (black) in a perturbed uniform distribution"
    )
    plt.tight_layout()
