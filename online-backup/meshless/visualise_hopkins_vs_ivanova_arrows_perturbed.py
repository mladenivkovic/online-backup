...
def main():
    ...
    print(
        "Sum Hopkins:",
        np.sum(A_ij_Hopkins, axis=0),
        "max:",
        np.absolute(A_ij_Hopkins[:, 0]).max(),
        np.absolute(A_ij_Hopkins[:, 1]).max(),
    )
    print(
        "Sum Ivanova:",
        np.sum(A_ij_Ivanova, axis=0),
        "max:",
        np.absolute(A_ij_Ivanova[:, 0]).max(),
        np.absolute(A_ij_Ivanova[:, 1]).max(),
    )
